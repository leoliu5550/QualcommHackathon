...
class TestPathAssembly:
    ...
    def test_assemble_windows_path(self, mock_logger, parser):
        ...
        This test specifically validates Windows path handling using PureWindowsPath
        to ensure cross-platform compatibility. When running on Unix-based CI systems,
        the parser must correctly extract filenames from Windows path strings like
        "C:\\Users\\John\\Desktop\\report.pdf".
        """
        # Windows path with backslashes - this is a Windows path string
        windows_path = "C:\\Users\\John\\Desktop\\report.pdf"
        llm_output = json.dumps({windows_path: {"category": "Reports", "summary": "Annual report", "reason": "Business"}})

        result = parser.parse(llm_output)

        mapping = result[windows_path]
        assert mapping.old_path == windows_path
        # Expected: only the filename "report.pdf" should be extracted
    ...
    @patch("fileorg.llm_classifier.adapters.output_parsers.path_mapping_output_parser.logger")
    def test_windows_path_with_forward_slashes(self, mock_logger, parser):
        """Should handle Windows paths with forward slashes (common in code)."""
        # Windows path with forward slashes - still a Windows path due to drive letter
        windows_path = "C:/Users/John/Desktop/report.pdf"
        llm_output = json.dumps({windows_path: {"category": "Reports", "summary": "Report", "reason": "Document"}})
    ...
    @patch("fileorg.llm_classifier.adapters.output_parsers.path_mapping_output_parser.logger")
    def test_unix_path_extraction(self, mock_logger, parser):
        """Should correctly extract filename from Unix paths."""
        unix_path = "/home/user/documents/report.pdf"
        llm_output = json.dumps({unix_path: {"category": "Reports", "summary": "Report", "reason": "Document"}})

        result = parser.parse(llm_output)

        mapping = result[unix_path]
        assert mapping.old_path == unix_path
        assert mapping.new_relative_path == "Reports/report.pdf"
