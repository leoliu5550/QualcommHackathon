            ...
            if export_config.get("use_fixed_export", True):
                ...
                try:
                    onnx_path = export_model_to_onnx(
                        ...
                        output_dir=self.output_dir,
                        opset=export_config.get("opset", 14),
                        use_external_data_format=export_config.get(
...
def main():
    ...
    parser.add_argument(
        "--action",
        type=str,
        choices=["export", "upload", "full", "test"],
        default="full",
        help="執行動作",
    )
    parser.add_argument("--model-id", type=str, help="HuggingFace 模型 ID")
    parser.add_argument("--model-name", type=str, help="自定義模型名稱")
    parser.add_argument("--onnx-path", type=str, help="ONNX 模型路徑（用於上傳或測試）")
    parser.add_argument("--check-status", action="store_true", help="檢查 AI Hub 狀態")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print(
            "3. 上傳模型: python model_pipeline.py --action upload --onnx-path ./path/to/model.onnx"
        )
        print("4. 完整流程: python model_pipeline.py --action full")
        print(
            "5. 測試推理: python model_pipeline.py --action test --onnx-path ./path/to/model.onnx"
        )
        print("\n正在檢查系統狀態...")
