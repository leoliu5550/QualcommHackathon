...
class ReportGenerator:
    ...
    def _generate_markdown_report(self, file_data: Dict, summaries: Dict, output_file: str):
        ...
        for folder_name in sorted(folder_mappings.keys()):
            ...
            md_lines.extend([f"### {folder_name}", f"*共 {len(files)} 個檔案*", ""])
            ...
            for file_name in sorted(files):
                ...
                if file_name in summaries:
                    summary = (
                        summaries[file_name][:100] + "..."
                        if len(summaries[file_name]) > 100
                        else summaries[file_name]
                    )
    ...
    def _generate_statistics_report(
        self, folder_mappings: Dict, file_paths: List[Dict], output_file: str
    ):
        """生成統計報告"""
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            for folder_name, details in sorted(stats["folder_details"].items()):
                f.write(
                    f"{folder_name:<30} {details['file_count']:>5} 個檔案 ({details['percentage']:>5.1f}%)\n"
                )
