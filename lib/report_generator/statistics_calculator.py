...
class StatisticsCalculator:
    ...
    def calculate_statistics(self, folder_mappings: Dict[str, List[str]], 
                           file_paths: List[Dict]) -> Dict:
        """
        計算檔案整理統計資料
        ...
        for folder_name, files in folder_mappings.items():
            stats["folder_details"][folder_name] = {
                ...
                "percentage": round(len(files) / stats["total_files"] * 100, 1) if stats["total_files"] > 0 else 0
            }
    ...
    def generate_text_chart(self, folder_mappings: Dict[str, List[str]], max_width: int = 50) -> str:
        """
        生成文字統計圖表
        ...
        for folder_name, files in sorted_folders:
            ...
            chart_lines.append(
                f"{display_name:<25} {bar} {file_count:>3} ({percentage:>5.1f}%)"
            )
    ...
    def generate_summary(self, stats: Dict) -> str:
        ...
        summary_lines = [
            ...
            f"平均每個資料夾: {stats['total_files'] / stats['total_folders']:.1f} 個檔案" if stats['total_folders'] > 0 else "平均每個資料夾: 0 個檔案",
            "",
            "📁 資料夾大小分布:",
        ]
        
        if stats['largest_folder']:
            summary_lines.append(f"  最大: {stats['largest_folder'][0]} ({stats['largest_folder'][1]} 個檔案)")
        if stats['smallest_folder']:
            summary_lines.append(f"  最小: {stats['smallest_folder'][0]} ({stats['smallest_folder'][1]} 個檔案)")
