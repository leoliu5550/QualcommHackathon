...
def sample_files(temp_dir):
    ...
    files["json"].write_text(
        json.dumps({"key": "value", "數字": 123}, ensure_ascii=False), encoding="utf-8"
    )
...
def mock_file_paths_data():
    ...
    return {
        "file_paths": [
            {"original": "/test/source/file1.txt", "new": "/test/organized/category1/file1.txt"},
            {"original": "/test/source/file2.pdf", "new": "/test/organized/category2/file2.pdf"},
        ],
        "folder_mappings": {"category1": ["file1.txt"], "category2": ["file2.pdf"]},
...
def ai_test_config():
    """AI 模組測試專用配置"""
    return {"backend": "local", "model_id": "test-model", "device": "cpu", "max_new_tokens": 100}


@pytest.fixture
def parser_test_config():
    """Parser 模組測試專用配置"""
    return {"char_limit": 1000, "encoding_fallback": ["utf-8", "cp1252", "latin-1"]}
...
def reporter_test_data():
    ...
    return {
        ...
        "folder_mappings": {"docs": ["file1.txt", "file2.pdf"]},
        "statistics": {"total_files": 2, "total_folders": 1, "success_rate": 100.0},
    }
...
def mock_organizer_dependencies():
    """模擬 Organizer 所有依賴"""
    with patch("fileorg.scanner.core.FileScanner") as mock_scanner, patch(
        "fileorg.parsers.manager.parser_manager"
    ) as mock_parser, patch("fileorg.classifier.classifier.create_name") as mock_classifier, patch(
        "fileorg.reporter.generator.ReportGenerator"
    ) as mock_reporter:
        ...
        mock_classifier.process_files.return_value = {
            "file_paths": [{"original": "/test/file1.txt", "new": "/test/docs/file1.txt"}]
        }
