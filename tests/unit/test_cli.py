...
class TestStandardMode:
    ...
    @patch("fileorg.core.organizer.Organizer")
    @patch("os.makedirs")
    def test_run_standard_mode_no_backup(
        self, mock_makedirs, mock_organizer_class, mock_check_backup
    ):
    ...
    def test_run_standard_mode_with_backup(
        self, mock_apply_backup, mock_load_backup, mock_check_backup
    ):
...
class TestApplyBackupStructure:
    ...
    def test_apply_backup_structure(self, mock_move, mock_makedirs, mock_exists):
        ...
        mock_move.assert_any_call(
            os.path.normpath("/original/file1.txt"), os.path.normpath("/organized/docs/file1.txt")
        )
...
class TestApplyBackupStructureExtended:
    ...
    def test_apply_backup_structure_missing_files(self, mock_print, mock_exists):
        ...
        backup_data = {
            "file_paths": [{"original": "/missing/file1.txt", "new": "/organized/docs/file1.txt"}]
        }
        ...
        warning_calls = [
            call for call in mock_print.call_args_list if "找不到原始檔案" in str(call)
        ]
    ...
    @patch("builtins.print")
    def test_apply_backup_structure_move_error(
        self, mock_print, mock_move, mock_makedirs, mock_exists
    ):
        """Test applying backup when file move fails"""
        backup_data = {
            "file_paths": [{"original": "/original/file1.txt", "new": "/organized/docs/file1.txt"}]
        }
    ...
    def test_apply_backup_structure_empty_data(self):
        ...
        summary_calls = [
            call for call in mock_print.call_args_list if "已根據備份結構移動" in str(call)
        ]
...
class TestRunStandardModeExtended:
    ...
    @patch("os.makedirs")
    @patch("builtins.print")
    def test_run_standard_mode_corrupted_backup(
        self, mock_print, mock_makedirs, mock_organizer_class, mock_load_backup, mock_check_backup
    ):
        ...
        corrupted_calls = [
            call for call in mock_print.call_args_list if "備份檔案損壞或空白" in str(call)
        ]
    ...
    @patch("fileorg.core.organizer.Organizer")
    @patch("os.makedirs", side_effect=OSError("Permission denied"))
    def test_run_standard_mode_makedirs_error(
        self, mock_makedirs, mock_organizer_class, mock_check_backup
    ):
    ...
    @patch("fileorg.core.organizer.Organizer")
    @patch("os.makedirs")
    def test_run_standard_mode_organizer_error(
        self, mock_makedirs, mock_organizer_class, mock_check_backup
    ):
...
class TestMainFunctionExtended:
    ...
    def test_main_nonexistent_path_message(self, mock_print, mock_exists):
        ...
        error_calls = [call for call in mock_print.call_args_list if "不存在" in str(call)]
        assert len(error_calls) > 0
    ...
    def test_main_exception_handling(
        self, mock_traceback, mock_print, mock_run_standard, mock_exists
    ):
        ...
        error_calls = [
            call for call in mock_print.call_args_list if "執行過程中發生錯誤" in str(call)
        ]
        assert len(error_calls) > 0
