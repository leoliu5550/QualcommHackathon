...
class TestRestoreSingleFile:
    ...
    @patch("shutil.move")
    @patch("builtins.print")
    def test_restore_single_file_success(
        self, mock_print, mock_move, mock_makedirs, mock_exists, tmp_path
    ):
    ...
    @patch("builtins.print")
    def test_restore_single_file_move_failure(
        self, mock_print, mock_move, mock_makedirs, mock_exists, tmp_path
    ):
    ...
    @patch("shutil.move")
    @patch("builtins.print")
    def test_restore_single_file_relative_path_conversion(
        self, mock_print, mock_move, mock_makedirs, mock_exists, tmp_path
    ):
...
class TestCleanupEmptyDirectories:
    ...
    def test_cleanup_empty_directories(
        self, mock_print, mock_rmdir, mock_listdir, mock_isdir, mock_exists, tmp_path
    ):
        """Test cleanup of empty directories"""
        restorer = FileRestorer(str(tmp_path))

        backup_data = {"folder_mappings": {"docs": ["file1.txt"], "images": ["photo.jpg"]}}
    ...
    @patch("os.rmdir")
    def test_cleanup_non_empty_directories(
        self, mock_rmdir, mock_listdir, mock_isdir, mock_exists, tmp_path
    ):
        """Test that non-empty directories are not removed"""
        restorer = FileRestorer(str(tmp_path))

        backup_data = {"folder_mappings": {"docs": ["file1.txt"]}}
...
class TestFullRestoreProcess:
    ...
    def test_restore_success(
        self, mock_print, mock_cleanup, mock_restore_single, mock_load_backup, tmp_path
    ):
    ...
    @patch.object(FileRestorer, "_restore_single_file")
    @patch("builtins.print")
    def test_restore_partial_success(
        self, mock_print, mock_restore_single, mock_load_backup, tmp_path
    ):
