...
class TestOrganizer:
    ...
    def test_move_files_success(self, temp_dir):
        ...
        organizer = Organizer()
        generate_result = {
            "file_paths": [{"original": str(source_file), "new": str(target_dir / "source.txt")}]
        }
    ...
    def test_move_file_permission_error(self, temp_dir, monkeypatch):
        ...
        generate_result = {
            "file_paths": [
                {"original": str(source_file), "new": str(temp_dir / "target" / "file.txt")}
            ]
        }
    ...
    def test_duplicate_filename_handling(self, temp_dir):
        ...
        generate_result = {
            "file_paths": [
                {"original": str(file1), "new": str(temp_dir / "target" / "same.txt")},
                {"original": str(file2), "new": str(temp_dir / "target" / "same.txt")},
            ]
        }
    ...
    def test_circular_path_prevention(self, temp_dir):
        ...
        # 嘗試將檔案移動到自己
        generate_result = {
            "file_paths": [{"original": str(source), "new": str(source)}]  # 相同路徑
        }
    ...
    def test_various_file_counts(self, temp_dir, file_count):
        ...
        for i in range(file_count):
            ...
            file_path.write_text(f"Content {i}")
            files.append(
                {"original": str(file_path), "new": str(temp_dir / "organized" / f"file_{i}.txt")}
            )
    ...
    def test_path_normalization(self, temp_dir):
        ...
        with patch("fileorg.core.organizer.create_name") as mock_create:
            mock_create.process_files.return_value = {
                "file_paths": [{"original": "./file.txt", "new": "./organized/file.txt"}]
            }
...
class TestOrganizerWorkflowIntegration:
    ...
    @patch("fileorg.core.organizer.create_name")
    @patch("fileorg.core.organizer.ReportGenerator")
    def test_start_organize_complete_workflow(
        self, mock_report_class, mock_create_name, mock_parser, mock_scanner_class, temp_dir
    ):
        ...
        mock_create_name_instance.process_files.return_value = {
            "file_paths": [
                {
                    "original": str(temp_dir / "file1.txt"),
                    "new": str(temp_dir / "docs" / "file1.txt"),
                },
                {
                    "original": str(temp_dir / "file2.pdf"),
                    "new": str(temp_dir / "docs" / "file2.pdf"),
                },
            ]
        }
...
class TestOrganizerFileHandling:
    ...
    def test_move_file_with_unicode_paths(self, temp_dir):
        ...
        organizer = Organizer()
        generate_result = {"file_paths": [{"original": str(source), "new": str(target_path)}]}
    ...
    def test_move_file_overwrite_existing(self, temp_dir):
        ...
        organizer = Organizer()
        generate_result = {"file_paths": [{"original": str(source), "new": str(existing)}]}
    ...
    def test_move_file_disk_space_error(self, temp_dir, monkeypatch):
        ...
        generate_result = {
            "file_paths": [
                {"original": str(source), "new": str(temp_dir / "target" / "source.txt")}
            ]
        }
